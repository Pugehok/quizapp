...
export const MainComponent = () => {
  const test = useSelector((state) => state.Quiz.QuizesState);
              ...
              {test.map((e) => {
                return (
                  <div key={e.id} className={s.Wrapper__quizCard}>
                    ...
                    {/* {console.log(e)} */}
                    <span> Количество вопросов:{e.Quizes.length}</span>
                    <button key={e._id} onClick={(event) => HandleClicker(event, e.id)}>
                      Приступить
                    </button>
                  </div>
                );
